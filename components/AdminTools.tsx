...
const AdminTools: React.FC<{ showNotification: (type: 'success' | 'error' | 'info', message: string) => void }> = ({ showNotification }) => {
  ...
  return (
    <div className="animate-fade-in">
      <h1 className="text-3xl font-bold mb-6 text-text-primary">Administrative Tools</h1>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Purge Messages */}
        <Card>
          <h2 className="text-xl font-semibold mb-4 text-text-primary">Purge Messages</h2>
          <p className="text-text-secondary mb-4">Delete a specified number of messages from a channel. Use with caution.</p>
          <div className="flex gap-2 items-center">
            <input
              ...
              className="w-24 p-2 rounded-xl bg-surface-border border-transparent focus:outline-none focus:ring-2 focus:ring-primary"
            />
        ...
        {/* Error Log Viewer */}
        <Card>
          <h2 className="text-xl font-semibold mb-4 text-text-primary">Error Log Viewer</h2>
          <p className="text-text-secondary mb-4">Retrieve details for a specific error code.</p>
          <div className="flex gap-2 items-center">
            <input
              type="text"
          ...
          {foundError && (
            <div className="mt-4 p-3 bg-surface-border/50 rounded-lg">
              <p><strong className="text-error">Code:</strong> {foundError.code}</p>
              <p><strong className="text-text-secondary">Message:</strong> {foundError.message}</p>
              <p><strong className="text-text-tertiary">Timestamp:</strong> {foundError.timestamp}</p>
            </div>
          )}
        </Card>
      </div>
      ...
      <div className="mt-8">
          <Card className="border-error border-2">
            ...
            <div className="mt-4 flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                    <h3 className="font-bold text-text-primary">Gracefully Restart Bot</h3>
                    <p className="text-text-secondary">This will immediately restart the bot's process. Use this only if the bot is unresponsive.</p>
                </div>
       ...
       <Modal isOpen={isCrashModalOpen} onClose={() => setIsCrashModalOpen(false)} title="Confirm System Restart">
        <p className="mb-4 text-text-secondary">This is a critical action. To proceed, please type <strong className="text-error">RESTART</strong> into the box below.</p>
        <input
          type="text"
          className="w-full p-2 rounded-xl bg-background border-surface-border mb-4 focus:outline-none focus:ring-2 focus:ring-error text-text-primary"
          value={crashConfirm}
